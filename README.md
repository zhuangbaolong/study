...
1. 单例（懒汉式和饿汉式）
   1. 区别：
      ...
      4. 懒汉式，会延迟加载，在第一次使用本单例时才会出现实例对象，第一次调用时要做初始化，如果要做的工作比较多，性能会有些延迟，以后就像饿汉式一样。
...
1. 使用
   ...
   4. 不希望某个局部变量被修改。
2. 注意
   1. final修饰的属性是常量，修饰的属性在定义时，必须赋初值，并不能被修改，赋值可以赋在以下位置。
      1. 定义时
      2. 构造器中
      3. 代码块中
   2. 如果final修饰的属性是静态的，初始化的位置是：
      1. 定义时
      2. 静态代码块
   3. final类不能被继承，但是可以被实例化对象。
   4. 不是final类，但是含有final方法，则该方法不能被重写，但是可以被继承。
   5. 不能修饰构造器。
   6. final和static往往搭配使用，效率更高，不会被类加载。
